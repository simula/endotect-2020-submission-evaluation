...
def read_gt(gt_path):
    gt_classes = {
        ...
        "short-segment-barretts": 16,
        "ulcerative-colitis-0-1": 17,
        "ulcerative-colitis-1-2": 18,
        "ulcerative-colitis-2-3": 19,
        "ulcerative-colitis-grade-1": 20,
        "ulcerative-colitis-grade-2": 21,
        "ulcerative-colitis-grade-3": 22
    }
    gt_results = {}
    gt_classes_list = []
...
def evaluate_submission(submission_path):
    ...
    TP = np.diag(confusion_matrix)
    TN = confusion_matrix.sum() - (FP + FN + TP)
    PREC = metrics.precision_score(y_truth, y_pred, labels=list(gt_classes.values()), average=None)
    REC  = metrics.recall_score(y_truth, y_pred, labels=list(gt_classes.values()), average=None)
    F1   = metrics.f1_score(y_truth, y_pred, labels=list(gt_classes.values()), average=None)
    SPEC = TN / (TN + FP)
